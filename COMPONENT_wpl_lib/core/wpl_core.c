...
void wpl_start( )
{
    ...
    {
        WPRINT_LIB_ERROR(("CyPE wpl_start: Failed to init\n"));
        return;
    }

    if( wpl_cpl_init( ) != WICED_SUCCESS)
    {
        WPRINT_LIB_ERROR(("CyPE wpl_start: Failed to init CPL\n"));
        return;
    }
...
wiced_result_t handle_get_events_cmd( wpl_packet_t* request_packet )
{
    ...
    for ( i = 0; i < registered_cpl_count; i++ )
    {
        ...
        for ( j = 0; j < cpl_list[i]->proc_count; j++ )
        {
            ...
            /* Get the count of events from CPL */
            result = cpl_list[i]->cpl_callback( CPL_GET_EVENTS_LIST, &count, &cpl_proc, NULL );

            if ( result != WICED_SUCCESS )
                return result;

 //           WICED_BT_TRACE("handle_get_events_cmd: count: %d\n", count);
            ...
            event_list = out_packet->payload_start + PID + EID;

            /* Get the list of events from CPL  */
            //memcpy( out_packet->payload_start + PID + EID, out_data, count );
            result = cpl_list[i]->cpl_callback( CPL_GET_EVENTS_LIST, &count, &cpl_proc, &event_list );
...
wiced_result_t handle_get_event_descriptor_cmd( wpl_packet_t* request_packet )
{
    ...
    for ( i = 0; i < registered_cpl_count; i++ )
    {
        ...
        for ( j = 0; j < cpl_list[i]->proc_count; j++ )
        {
            ...
            /* CPL would give the number of events descriptors and list. First get the count */

            /* Get the count of events descriptors from CPL */
            result = cpl_list[i]->cpl_callback( CPL_GET_EVENTS_DESC_LIST, &count, request_packet->payload_start, NULL );
            if ( result != WICED_SUCCESS )
            {
                return result;
            }
            ...
            /* Copy count of events descriptors */
            memcpy( out_packet->payload_start + PID + EID, &count, PACKET_COUNT_SIZE );

            /* get the list of event descriptors */
            out_data = out_packet->payload_start + PID + EID + EDID;
