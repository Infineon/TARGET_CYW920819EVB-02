...
void wpl_transport_uart_thread_main( uint32_t arg )
{
    ...
    WPRINT_LIB_INFO( ( "CyPE: wpl_transport_uart_thread_main: enter\n" ) );
...
wiced_result_t wpl_uart_transport_send_packet( wpl_packet_t* packet )
{
    ...
    else
    {
        ...
        if ( result == WICED_SUCCESS )
        {
            uint8_t pad_byte = PADDING_BYTE;
            uint8_t pad_counter = 0;
            for(; pad_counter < PADDING_SIZE; pad_counter++)
                platform_wpl_uart_transmit_bytes(&pad_byte, 1);
        }
    }
    if ( result != WICED_SUCCESS )
    {
